...
impl convert::From<lcb_error_t> for CouchbaseError {
    fn from(err: lcb_error_t) -> Self {
        match err {
            lcb_error_t_LCB_AUTH_ERROR => CouchbaseError::AuthFailed,
            lcb_error_t_LCB_DELTA_BADVAL => CouchbaseError::DeltaBadval,
            lcb_error_t_LCB_E2BIG => CouchbaseError::TooBig,
            lcb_error_t_LCB_EBUSY => CouchbaseError::ServerBusy,
            lcb_error_t_LCB_EINTERNAL => CouchbaseError::Internal,
            lcb_error_t_LCB_EINVAL => CouchbaseError::InvalidValue,
            lcb_error_t_LCB_ENOMEM => CouchbaseError::NoMemoryLeft,
            lcb_error_t_LCB_ERANGE => CouchbaseError::InvalidRange,
            lcb_error_t_LCB_ERROR => CouchbaseError::Generic,
            lcb_error_t_LCB_ETMPFAIL => CouchbaseError::TemporaryFailure,
            lcb_error_t_LCB_KEY_EEXISTS => CouchbaseError::KeyExists,
            lcb_error_t_LCB_KEY_ENOENT => CouchbaseError::KeyDoesNotExist,
            lcb_error_t_LCB_DLOPEN_FAILED => CouchbaseError::PluginLibraryNotFound,
            lcb_error_t_LCB_DLSYM_FAILED => CouchbaseError::PluginInitializerNotFound,
            lcb_error_t_LCB_NETWORK_ERROR => CouchbaseError::NetworkFailure,
            lcb_error_t_LCB_NOT_MY_VBUCKET => CouchbaseError::NotMyVBucket,
            lcb_error_t_LCB_NOT_STORED => CouchbaseError::NotStored,
            lcb_error_t_LCB_NOT_SUPPORTED => CouchbaseError::NotSupported,
            lcb_error_t_LCB_UNKNOWN_COMMAND => CouchbaseError::UnknownCommand,
            lcb_error_t_LCB_UNKNOWN_HOST => CouchbaseError::UnknownHost,
            lcb_error_t_LCB_PROTOCOL_ERROR => CouchbaseError::ProtocolError,
            lcb_error_t_LCB_ETIMEDOUT => CouchbaseError::TimedOut,
            lcb_error_t_LCB_CONNECT_ERROR => CouchbaseError::ConnectError,
            lcb_error_t_LCB_BUCKET_ENOENT => CouchbaseError::BucketDoesNotExist,
            lcb_error_t_LCB_CLIENT_ENOMEM => CouchbaseError::MemoryAllocationFailure,
            lcb_error_t_LCB_CLIENT_ENOCONF => CouchbaseError::RequestNotScheduled,
            lcb_error_t_LCB_EBADHANDLE => CouchbaseError::BadHandle,
            lcb_error_t_LCB_SERVER_BUG => CouchbaseError::ServerBug,
            lcb_error_t_LCB_PLUGIN_VERSION_MISMATCH => CouchbaseError::PluginVersionMismatch,
            lcb_error_t_LCB_INVALID_HOST_FORMAT => CouchbaseError::InvalidHostFormat,
            lcb_error_t_LCB_INVALID_CHAR => CouchbaseError::InvalidChar,
            lcb_error_t_LCB_DURABILITY_ETOOMANY => CouchbaseError::InvalidDurabilityRequirement,
            lcb_error_t_LCB_DUPLICATE_COMMANDS => CouchbaseError::DuplicateCommands,
            lcb_error_t_LCB_NO_MATCHING_SERVER => CouchbaseError::NoMatchingServer,
            lcb_error_t_LCB_BAD_ENVIRONMENT => CouchbaseError::BadEnvironment,
            lcb_error_t_LCB_BUSY => CouchbaseError::ClientBusy,
            lcb_error_t_LCB_INVALID_USERNAME => CouchbaseError::InvalidUsername,
            lcb_error_t_LCB_CONFIG_CACHE_INVALID => CouchbaseError::ConfigCacheInvalid,
            lcb_error_t_LCB_SASLMECH_UNAVAILABLE => CouchbaseError::SaslMechUnavailable,
            lcb_error_t_LCB_TOO_MANY_REDIRECTS => CouchbaseError::TooManyRedirects,
            lcb_error_t_LCB_MAP_CHANGED => CouchbaseError::MapChanged,
            lcb_error_t_LCB_INCOMPLETE_PACKET => CouchbaseError::IncompletePacket,
            lcb_error_t_LCB_ECONNREFUSED => CouchbaseError::ConnectionRefused,
            lcb_error_t_LCB_ESOCKSHUTDOWN => CouchbaseError::SocketShutdown,
            lcb_error_t_LCB_ECONNRESET => CouchbaseError::ConnectionReset,
            lcb_error_t_LCB_ECANTGETPORT => CouchbaseError::PortAllocationFailed,
            lcb_error_t_LCB_EFDLIMITREACHED => CouchbaseError::FileDescriptorLimitReached,
            lcb_error_t_LCB_ENETUNREACH => CouchbaseError::NetworkUnreachable,
            lcb_error_t_LCB_ECTL_UNKNOWN => CouchbaseError::ControlCommandUnknown,
            lcb_error_t_LCB_ECTL_UNSUPPMODE => CouchbaseError::ControlCommandUnsupported,
            lcb_error_t_LCB_ECTL_BADARG => CouchbaseError::ControlCommandBadArgument,
            lcb_error_t_LCB_EMPTY_KEY => CouchbaseError::EmptyKey,
            lcb_error_t_LCB_SSL_ERROR => CouchbaseError::EncryptionError,
            lcb_error_t_LCB_SSL_CANTVERIFY => CouchbaseError::EncryptionCannotVerify,
            lcb_error_t_LCB_SCHEDFAIL_INTERNAL => CouchbaseError::InternalScheduleFailure,
            lcb_error_t_LCB_CLIENT_FEATURE_UNAVAILABLE => CouchbaseError::ClientFeatureUnavailable,
            lcb_error_t_LCB_OPTIONS_CONFLICT => CouchbaseError::OptionsConflict,
            lcb_error_t_LCB_HTTP_ERROR => CouchbaseError::HttpError,
            lcb_error_t_LCB_DURABILITY_NO_MUTATION_TOKENS => {
                CouchbaseError::DurabilityNoMutationTokens
            }
            lcb_error_t_LCB_UNKNOWN_MEMCACHED_ERROR => CouchbaseError::UnknownStatusCode,
            lcb_error_t_LCB_MUTATION_LOST => CouchbaseError::MutationLost,
            lcb_error_t_LCB_SUBDOC_PATH_ENOENT => CouchbaseError::SubdocPathDoesNotExist,
            lcb_error_t_LCB_SUBDOC_PATH_MISMATCH => CouchbaseError::SubdocPathMismatch,
            lcb_error_t_LCB_SUBDOC_PATH_EINVAL => CouchbaseError::SubdocPathMalformed,
            lcb_error_t_LCB_SUBDOC_PATH_E2BIG => CouchbaseError::SubdocPathTooBig,
            lcb_error_t_LCB_SUBDOC_DOC_E2DEEP => CouchbaseError::SubdocExistingValueToeep,
            lcb_error_t_LCB_SUBDOC_VALUE_CANTINSERT => CouchbaseError::SubdocCannotInsert,
            lcb_error_t_LCB_SUBDOC_DOC_NOTJSON => CouchbaseError::SubdocExistingNotJson,
            lcb_error_t_LCB_SUBDOC_NUM_ERANGE => CouchbaseError::SubdocNumericValueTooLarge,
            lcb_error_t_LCB_SUBDOC_BAD_DELTA => CouchbaseError::SubdocBadDelta,
            lcb_error_t_LCB_SUBDOC_PATH_EEXISTS => CouchbaseError::SubdocPathExists,
            lcb_error_t_LCB_SUBDOC_MULTI_FAILURE => CouchbaseError::SubdocMultiFailure,
            lcb_error_t_LCB_SUBDOC_VALUE_E2DEEP => CouchbaseError::SubdocValueTooDeepToInsert,
            lcb_error_t_LCB_EINVAL_MCD => CouchbaseError::InvalidPacket,
            lcb_error_t_LCB_EMPTY_PATH => CouchbaseError::SubdocEmptyPath,
            lcb_error_t_LCB_UNKNOWN_SDCMD => CouchbaseError::SubdocUnknownCommand,
            lcb_error_t_LCB_ENO_COMMANDS => CouchbaseError::NoCommandsSpecified,
            lcb_error_t_LCB_QUERY_ERROR => CouchbaseError::QueryError,
            lcb_error_t_LCB_GENERIC_TMPERR => CouchbaseError::GenericTmpError,
            lcb_error_t_LCB_GENERIC_SUBDOCERR => CouchbaseError::GenericSubdocError,
            lcb_error_t_LCB_GENERIC_CONSTRAINT_ERR => CouchbaseError::GenericConstraintError,
            lcb_error_t_LCB_NAMESERVER_ERROR => CouchbaseError::NameserverError,
            lcb_error_t_LCB_NOT_AUTHORIZED => CouchbaseError::NotAuthorized,
            lcb_error_t_LCB_MAX_ERROR => panic!("MAX_ERROR is internal!"),
            lcb_error_t_LCB_SUCCESS => panic!("SUCCESS is not an Error!"),
            lcb_error_t_LCB_AUTH_CONTINUE => {
