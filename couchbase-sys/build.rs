...
fn main() {
    let lcb_dir = "libcouchbase-2.7.4";
    ...
    let pkg_success = if cfg!(feature = "build-lcb") {
        ...
        let result = pkg_config::Config::new().atleast_version("2.7.4").probe("libcouchbase");
        if result.is_ok() {
            bindgen_path = Some(String::from(result.unwrap()
                .include_paths
                .get(0)
    ...
    if !pkg_success {
        if cfg!(feature = "build-lcb") {
            ...
            panic!("Need to build libcouchbase, but the 'build-lcb' feature is not enabled!");
        }
    }
    ...
    if cfg!(feature = "generate-binding") {
        ...
        let src_path = format!("{}/src/bindings-{}.rs", env!("CARGO_MANIFEST_DIR"), "2.7.4");
        println!("{}", src_path);
        let dst_path = format!("{}/bindings.rs", out_dir);
        fs::copy(&src_path, &dst_path).unwrap();
        // copy the pre-generated binding in place if it can be found
        // if it can't be found print error to say please run generate-binding feature
    }
}
