        ...
        D: Dispatcher,
    {
        ...
        if let Some(req) = opts.get_error_map {
            ...
            .await
            {
                Ok(r) => Some(r),
                Err(e) => {
                    warn!(context=dispatcher.log_context(), e:err; "Get error map failed {} {}", dispatcher.log_context(), e);
                    None
                }
            };
        };
        if let Some(req) = opts.auth {
                ...
                .await
            {
                Ok(_) => {}
                Err(e) => {
                    warn!(context=dispatcher.log_context(), e:err; "Auth failed {} {}", dispatcher.log_context(), e);
                    return Err(e);
                }
            };
        }
        ...
        if let Some(req) = opts.select_bucket {
                ...
                .await
            {
                Ok(r) => Some(r),
                Err(e) => {
                    warn!(context=dispatcher.log_context(), e:err; "Select bucket failed {} {}", dispatcher.log_context(), e);
                    return Err(e);
                }
            };
        }
