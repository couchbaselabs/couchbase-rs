...
impl From<OrphanReporterOptions> for couchbase_core::options::agent::OrphanReporterConfig {
    fn from(opts: OrphanReporterOptions) -> Self {
        let mut core_opts = couchbase_core::options::agent::OrphanReporterConfig::default();
        if let Some(enabled) = opts.enabled {
            core_opts = core_opts.enabled(enabled);
        }
